...
class postProcess:
    ...
    def __init__(self, config_file=os.getcwd() + "/Forget/config/default_config.ini"):
        ...
        for job in self.reader.jobs:  # remember to put everything into forgetdata
            clone_subdir = [
                ...
                + "/clones/"
                for f in os.scandir(
                    str(parent_dir_path) + "/" + self.exp_name + "/" + job + "/"
                )
                if f.is_dir()
            ]
            model_subdir = [
                str(parent_dir_path) + "/" + self.exp_name + "/" + job + "/" + f.name
                for f in os.scandir(
                    str(parent_dir_path) + "/" + self.exp_name + "/" + job + "/"
                )
                if f.is_dir()
            ...
            for dir in model_subdir:
                self.num_examples.append(
                    torch.load(dir + "/forgetdata/num_forgotten.pt")[1]
                )
            ...
            if self.reader.jobs[job]["model parameters"] == "default":
                self.model_hparams = hparams.ModelHparams(
        ...
        for clone_idx, clone_dir in enumerate(self.list_clone_folders):
            ...
            )  # torch.load(self.list_model_folders[clone_idx] + "/forgetdata/trainset.pt")
            self.forgot_correct_mask = torch.load(
                self.list_model_folders[clone_idx]
                + "/forgetdata/forgetmask_correct_epoch="
                + str(self.max_epoch)
                + ".pt"
            )
            self.forgot_correct_dataset = torch.utils.data.Subset(
                self.train_set, self.forgot_correct_mask
            )
            self.forget_correct_dataloader = torch.utils.data.DataLoader(
                self.forgot_correct_dataset, batch_size=128
            )  # to change: this call to batch_size
            self.model_list = []
            ...
            catalog = self.classifyDataset(
                self.forget_correct_dataloader,
                self.model_list,
                self.num_examples[clone_idx],
            )
            torch.save(catalog, self.list_model_folders[clone_idx] + "/catalog.pt")

            self.forget_correct_stats = torch.load(
                self.list_model_folders[clone_idx]
                + "/forgetdata/forgotten_correct_stats_epoch="
                + str(self.max_epoch)
                + ".pt"
            )
            self.epsilonForgotten, self.timesForgotten = self.tabulateNoiseForget(
                catalog, self.epsilons, self.forget_correct_stats
            )
            self.totalEpsilons.append(self.epsilonForgotten)
            self.totalForgotten.append(self.timesForgotten)
            torch.save(
                torch.tensor(self.epsilonForgotten),
                self.list_model_folders[clone_idx] + "/epsilonForgotten.pt",
            )
            torch.save(
                torch.tensor(self.timesForgotten),
                self.list_model_folders[clone_idx] + "/timesForgotten.pt",
            )

            # print(self.totalEpsilons)

            self.totalEpsilonsTensor = torch.flatten(
                torch.Tensor(
                    [item for sublist in self.totalEpsilons for item in sublist]
                )
            )
            self.totalForgottenTensor = torch.flatten(
                torch.Tensor(
                    [item for sublist in self.totalForgotten for item in sublist]
                )
            )

            torch.save(
                self.totalEpsilonsTensor,
                self.list_model_folders[clone_idx] + "/epsilontotal.pt",
            )
            torch.save(
                self.totalForgottenTensor,
                self.list_model_folders[clone_idx] + "/timesforgottentotal.pt",
            )
    ...
    def tabulateNoiseForget(self, catalog, epsilonList, forgetCorrectStats):
        ...
        for k in range(len(catalog[0])):  # go through each example
            idx = next(
                (i for i in range(len(catalog[0:, k])) if catalog[0:, k][i] == 0), None
            )
            if idx != None:
    ...
    def findLargestEpsilon(self, epsilonForgotten, timesForgotten, largestN):
        ...
        for j in range(smallest_value, largest_value + 1):
            ...
            for k in range(len(timesForgotten)):
                ...
                largest_epsilon[j - smallest_value, 0:] = torch.tensor(
                    heapq.nlargest(largestN, [epsilonForgotten[i] for i in idx])
                )

        return torch.flatten(largest_epsilon), torch.flatten(largest_forgotten)


def getEpsilons(num_points=200, min_noise=0.0, max_noise=0.1):
